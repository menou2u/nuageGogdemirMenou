...
public class Nuage {
	...
	public static void main(String[] args) {
		 ...
		 * toURI(). getPath()); } catch (URISyntaxException e1) {
		 * e1.printStackTrace(); } /*JFrame frame = new JFrame(); JPanel p = new
		 * JPanel(); p.add(new JLabel(f.getParentFile().getPath()));
		 * 
		 * frame.add(p); frame.setVisible(true);
		 * frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); frame.pack();
		 */
		/*
		 * File giac = new File(f.getAbsolutePath(),
		 * "\\Resources\\javagiac.dll");
