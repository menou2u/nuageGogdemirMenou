...
public class Visual extends JFrame implements KeyListener, AdjustmentListener, Runnable, WindowListener, ActionListener,
	...
	/** Labels */
	private String[] FbLabel = { "root", "lcm", "gcd", "rand", "fact", "sipl", "fmax", "fmin", "perm", "+inf", "-inf",
			"limt", "canf", "solv", "diff", "intg", "sum", "lin" };
	/** Input String */
	private String[] FbInput = { "root(3,?)", "lcm(,)", "gcd(,)", "rand()", "factor(?)", "simplify(?)", "fmax(?,?)",
			"fmin(?,?)", "perm(,)", "+infinity", "-infinity", "limit(?, ?=0)", "canonical_form(?)", "solve(?=0, ?)",
			"function_diff(?)(?)", "integrate(?,?)", "sum(?, ?, 1, +infinity)", "lin(?)" };
	static {
		try {
	...
	/**
	 * Windows mechanism to load library: javagiac For windows you must run in this
	 * way: java -Djava.library.path=C:\tmp\javagiac javagiac.Visual where
	...
	public Visual() {
		...
		jsrcoll = new JScrollPane(ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED,
	...
	public JLabel newJLabelResult() {
		JLabel res = new JLabel("", SwingConstants.RIGHT);
		res.setFont(fontList);
		return res;
	}
