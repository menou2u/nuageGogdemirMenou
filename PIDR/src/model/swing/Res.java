...
public class Res  {
	...
	private AWTChart getDemoChart(JavaFXChartFactory factory, String toolkit) {
        ...
        Mapper mapper = new Mapper() {
            @Override
            public double f(double x, double y) {
                return -0.368460936989*Math.pow(Math.atan(3.0*x+4.0*y),3)-0.131665897037*Math.pow(Math.atan(x+6.0*y),5)-0.0217366824047*Math.atan(x*y)+1.29289710213*Math.atan(3.0*x+y)-Math.pow(Math.pow(6.30754609552, -5)*x*y, 2)+Math.pow(Math.pow(6.19618433736, -6)*(x*y),2)-0.00047678180653*x*y+0.72501413259;
            }
        };
